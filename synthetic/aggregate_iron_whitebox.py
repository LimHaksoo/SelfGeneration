#!/usr/bin/env python3
"""Aggregate synthetic IRON AUP, AUR, CE, and NCE across five folds."""
...
def latex(mean: float, std: float) -> str:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    for raw_data in args.datasets:
        ...
        for fold in args.folds:
            ...
            if payload.get("status") != "complete" or not isinstance(
                ...
                missing.append(f"{path}: incomplete or missing completeness")
                continue
            ...
            values = np.asarray(
                [aup_value, aur_value, ce_value, nce_value],
                dtype=np.float64,
            )
            if not np.isfinite(values).all():
                missing.append(f"{path}: non-finite metric")
                continue
        ...
        aup_mean, aup_std = mean_std(aup_values)
        aur_mean, aur_std = mean_std(aur_values)
        ce_mean, ce_std = mean_std(ce_values)
        nce_mean, nce_std = mean_std(nce_values)
        ...
        summary_rows.append(
            {
                ...
                "NCE_mean": nce_mean,
                "NCE_std": nce_std,
                "AUP_latex": latex(aup_mean, aup_std),
                "AUR_latex": latex(aur_mean, aur_std),
                "CE_latex": latex(ce_mean, ce_std),
                "NCE_latex": latex(nce_mean, nce_std),
            }
        )
    ...
    if missing:
        print(f"[WARN] missing or invalid results: {len(missing)}")
    return 0
