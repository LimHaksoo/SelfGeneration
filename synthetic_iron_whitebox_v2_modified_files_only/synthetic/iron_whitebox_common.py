...
def _project_data_dir(name: str) -> str:
    """Return an absolute data path.

    Tint HMM constructs an intermediate filename containing ``data_dir`` and
    then joins that filename with ``data_dir`` again.  A relative path such as
    ``data/hmm`` therefore becomes ``data/hmm/data/hmm``.  With an absolute
    path, ``os.path.join`` correctly keeps the absolute second operand.
    """

    destination = (PROJECT_ROOT / "data" / str(name)).resolve()
...
def _preprocess_or_prepare(datamodule, split: str) -> Dict[str, Tensor]:
    try:
        ...
        data_dir = getattr(datamodule, "data_dir", None)
        if data_dir:
        ...
        try:
            ...
            raise FileNotFoundError(
                ...
                f"data_dir={data_dir!r}. Initial error: {first_error}"
            ) from second_error
...
def load_bundle(data: str, fold: int, seed: int) -> SyntheticBundle:
    ...
    if dataset == "state":
        ...
        datamodule = HMM(
            ...
            fold=int(fold),
            seed=int(seed),
            data_dir=_project_data_dir("hmm"),
        )
        ...
        datamodule = Switch(
            ...
            fold=int(fold),
            seed=int(seed),
            data_dir=_project_data_dir("switchstate"),
        )
